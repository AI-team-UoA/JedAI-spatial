...
import dit.anonymous.webapp.utilities.Reader;
...
public class ExecutionController {
		...
		try {
			...
			switch (WorkflowManager.wf_mode) {
				case Options.WORKFLOW_PROGRESSIVE:
					WorkflowManager.setPrioritizationMethod((MethodModel) wfConfig.get(Options.PRIORITIZATION));
				case Options.WORKFLOW_BLOCKING_BASED:
							
					WorkflowManager.setSchemaClustering((MethodModel) wfConfig.get(Options.SCHEMA_CLUSTERING));
					
					List<MethodModel> bb = (List<MethodModel>) wfConfig.get(Options.BLOCK_BUILDING);
					for (MethodModel m : bb) WorkflowManager.addBlockBuildingMethod(m);
					
					if (wfConfig.containsKey(Options.BLOCK_CLEANING)){
						List<MethodModel> bc = (List<MethodModel>) wfConfig.get(Options.BLOCK_CLEANING);
						for (MethodModel m : bc) WorkflowManager.addBlockCleaningMethod(m);
					}
					
					WorkflowManager.setComparisonCleaning((MethodModel) wfConfig.get(Options.COMPARISON_CLEANING));
					WorkflowManager.setEntityMatching((MethodModel) wfConfig.get(Options.ENTITY_MATCHING));
					WorkflowManager.setEntityClustering((MethodModel) wfConfig.get(Options.ENTITY_CLUSTERING));
					break;

				case Options.WORKFLOW_JOIN_BASED:

					WorkflowManager.setSimilarityJoinMethod((SimilarityMethodModel) wfConfig.get(Options.SIMILARITY_JOIN));
					WorkflowManager.setEntityClustering((MethodModel) wfConfig.get(Options.ENTITY_CLUSTERING));
					break;
	...
	public boolean configurationsSetCorrectly() {
		...
		boolean datasetOk;
		if (WorkflowManager.er_mode.equals(Options.DIRTY_ER))
		...
		this.methodsConfig = getWorkflowConfigurations(WorkflowManager.workflowConfigurationsID);
		 ...
		 for (String key : methodsConfig.keySet())  {
			 ...
			 if (value instanceof MethodModel ) {
				 ...
				 automatic_conf = automatic_conf || conf.equals(Options.AUTOMATIC_CONFIG);
	...
	public Triplet<ClustersPerformance, Double, Integer> executeWorkflow(
		...
		try {
			...
			if(this.anyAutomaticConfig()) {
				if(automatic_type.equals(Options.AUTOCONFIG_HOLISTIC)) {
					...
					double auc = -1d;
					if (WorkflowManager.wf_mode.equals(Options.WORKFLOW_PROGRESSIVE)){
					...
					performances =  exec
							.submit(() -> { return WorkflowManager.runStepByStepWorkflow(methodsConfig, 
									search_type.equals(Options.AUTOCONFIG_RANDOMSEARCH), interrupt_execution);}
							)
							.get();
