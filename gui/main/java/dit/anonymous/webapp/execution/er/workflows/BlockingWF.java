package dit.anonymous.webapp.execution.er.workflows;
...
import dit.anonymous.webapp.execution.er.BlockUtils;
    ...
    runStepByStepWorkflow(boolean random, Map<String, Object> methodsConfig, AtomicBoolean interrupted) {
		...
		try {
		    ...
		    if (block_building != null && !block_building.isEmpty()) {	
                ...
                List<MethodModel> bb_methods = (List<MethodModel>) methodsConfig.get(Options.BLOCK_BUILDING);
		    	...
		    	for (IBlockBuilding bb : block_building) {
		            ...
		            if (bb_methods.get(index).getConfiguration_type().equals(Options.AUTOMATIC_CONFIG)) {
		    ...
		    if (block_cleaning != null && !block_cleaning.isEmpty()) {
		    	...
		    	List<MethodModel> bp_methods = (List<MethodModel>) methodsConfig.get(Options.BLOCK_CLEANING);
		    	int index = 0;
		        for (IBlockProcessing bp: block_cleaning) {	
		            ...
		            if (bp_methods.get(index).getConfiguration_type().equals(Options.AUTOMATIC_CONFIG)) {
		                // Optimize the method
		    ...
		    MethodModel cc_method = (MethodModel) methodsConfig.get(Options.COMPARISON_CLEANING);
		    if (cc_method.getConfiguration_type().equals(Options.AUTOMATIC_CONFIG)) {
		    ...
		    time1 = System.currentTimeMillis();
		    MethodModel em_method = (MethodModel) methodsConfig.get(Options.ENTITY_MATCHING);
		    MethodModel ec_method = (MethodModel) methodsConfig.get(Options.ENTITY_CLUSTERING);
		    boolean matchingAutomatic = em_method.getConfiguration_type().equals(Options.AUTOMATIC_CONFIG);
		    boolean clusteringAutomatic = ec_method.getConfiguration_type().equals(Options.AUTOMATIC_CONFIG);
