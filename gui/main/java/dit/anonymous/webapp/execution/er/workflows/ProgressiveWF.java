...
import dit.anonymous.webapp.datatypes.WorkflowResult;
...
public class ProgressiveWF {
    ...
    public static boolean configurationOk(){
		...
		boolean prioritizationBlocks = prioritizationModel.getLabel().equals(Options.GLOBAL_PROGRESSIVE_SORTED_NEIGHBORHOOD) || prioritizationModel.getLabel().equals(Options.LOCAL_PROGRESSIVE_SORTED_NEIGHBORHOOD);
	...
	run(boolean final_run, AtomicBoolean interrupted)  {
		...
		try {	
			...
			// Prioritization
			overheadStart = System.currentTimeMillis();
			boolean isDirtyEr = WorkflowManager.er_mode.equals(Options.DIRTY_ER);
	...
	public static void setPrioritization(long budget){
		if (!prioritizationModel.getConfiguration_type().equals(Options.MANUAL_CONFIG)) {
