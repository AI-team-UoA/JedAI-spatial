...
import dit.anonymous.webapp.execution.er.WorkflowManager;
...
public class MethodConfigurations {
    ...
    private static Map<String, BlockBuildingMethod> createMap() {
        ...
        result.put(Options.STANDARD_TOKEN_BUILDING, BlockBuildingMethod.STANDARD_BLOCKING);
        result.put(Options.SORTED_NEIGHBORHOOD, BlockBuildingMethod.SORTED_NEIGHBORHOOD);
        result.put(Options.SORTED_NEIGHBORHOOD_EXTENDED, BlockBuildingMethod.EXTENDED_SORTED_NEIGHBORHOOD);
        result.put(Options.Q_GRAMS_BLOCKING, BlockBuildingMethod.Q_GRAMS_BLOCKING);
        result.put(Options.Q_GRAMS_BLOCKING_EXTENDED, BlockBuildingMethod.EXTENDED_Q_GRAMS_BLOCKING);
        result.put(Options.SUFFIX_ARRAYS_BLOCKING, BlockBuildingMethod.SUFFIX_ARRAYS);
        result.put(Options.SUFFIX_ARRAYS_BLOCKING_EXTENDED, BlockBuildingMethod.EXTENDED_SUFFIX_ARRAYS);
        result.put(Options.LSH_SUPERBIT_BLOCKING, BlockBuildingMethod.LSH_SUPERBIT_BLOCKING);
        result.put(Options.LSH_MINHASH_BLOCKING, BlockBuildingMethod.LSH_MINHASH_BLOCKING);
	...
	public static ISchemaClustering getSchemaClusteringMethodByName(String methodName) {
        ...
        switch (methodName) {
            case Options.ATTRIBUTE_NAME_CLUSTERING:
                ...
                break;
            case Options.ATTRIBUTE_VALUE_CLUSTERING:
                schemaClustering = new AttributeValueClustering(
                        RepresentationModel.CHARACTER_TRIGRAMS, SimilarityMetric.ENHANCED_JACCARD_SIMILARITY);
                break;
            case Options.HOLISTIC_ATTRIBUTE_CLUSTERING:
	...
	public static IEntityClustering getEntityClusteringMethod(String methodName) {
        ...
        switch (methodName) {
            case Options.CENTER_CLUSTERING:
                method = new CenterClustering();
                break;
            case Options.CONNECTED_COMPONENTS_CLUSTERING:
                method = new ConnectedComponentsClustering();
                break;
            case Options.CUT_CLUSTERING:
                method = new CutClustering();
                break;
            case Options.CORRELATION_CLUSTERING:
                method = new CorrelationClustering();
                break;
            case Options.MARKOV_CLUSTERING:
                method = new MarkovClustering();
                break;
            case Options.MERGE_CENTER_CLUSTERING:
                method = new MergeCenterClustering();
                break;
            case Options.RICOCHET_SR_CLUSTERING:
                method = new RicochetSRClustering();
                break;
            case Options.UNIQUE_MAPPING_CLUSTERING:
                method = new UniqueMappingClustering();
                break;
            case Options.ROW_COLUMN_CLUSTERING:
