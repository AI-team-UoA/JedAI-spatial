...
import utils.configuration.Constants.GeoSparkGridType.GeoSparkGridType
...
import utils.configuration.Constants.LocationSparkGridType.LocationSparkGridType
...
import utils.configuration.Constants.SedonaGridType.SedonaGridType
...
import utils.configuration.Constants.{SpatialSparkPartitionMethod, _}
...
case class ConfigurationErrorMessage(message: String){
    def getMessage: String = s"GeoLinker: ERROR - $message"
}
...
sealed trait ConfigurationT {
    ...
    // Spatial Spark
    def getSpatialSparkMethod: SpatialSparkPartitionMethod = SpatialSparkPartitionMethod.withName(configurations.getOrElse(InputConfigurations.CONF_SPATIAL_SPARK_METHOD, SpatialSparkPartitionMethod.FGP.toString))

    def getSpatialSparkMethodConf: String = configurations.getOrElse(InputConfigurations.CONF_SPATIAL_SPARK_METHOD_CONF, "512:512")

    def getSpatialSparkExtent: String = configurations.getOrElse(InputConfigurations.CONF_SPATIAL_SPARK_EXTENT, "")

    def getRadius: Double = configurations.getOrElse(InputConfigurations.CONF_SPATIAL_SPARK_RADIUS, "0.0").toDouble

    // Location Spark
    def getLocationSparkGridType: LocationSparkGridType = LocationSparkGridType.withName(configurations.getOrElse(InputConfigurations.CONF_LOCATION_SPARK_GRID_TYPE, "QUADTREE"))

    def getLocationSparkLocalIndexType: LocationSparkLocalIndex = LocationSparkLocalIndex.withName(configurations.getOrElse(InputConfigurations.CONF_LOCATION_SPARK_LOCAL_INDEX_TYPE, LocationSparkLocalIndex.RTREE.toString))

    // Magellan
    def getZOrderCurvePrecision: Int = configurations.getOrElse(InputConfigurations.CONF_MAGELLAN_Z_ORDER_CURVE_PRECISION, "20").toInt

    // GeoSpark
    def getGeoSparkLocalIndexType: GeoSparkLocalIndex = GeoSparkLocalIndex.withName(configurations.getOrElse(InputConfigurations.CONF_GEO_SPARK_LOCAL_INDEX_TYPE, "NO_INDEX"))

    def getGeoSparkGridType: GeoSparkGridType = GeoSparkGridType.withName(configurations.getOrElse(InputConfigurations.CONF_GEO_SPARK_GRID_TYPE, "QUADTREE"))

    // Apache Sedona
    def getSedonaLocalIndexType: SedonaLocalIndex = SedonaLocalIndex.withName(configurations.getOrElse(InputConfigurations.CONF_SEDONA_LOCAL_INDEX_TYPE, "NO_INDEX"))

    def getSedonaGridType: SedonaGridType = SedonaGridType.withName(configurations.getOrElse(InputConfigurations.CONF_SEDONA_GRID_TYPE, "QUADTREE"))
}
