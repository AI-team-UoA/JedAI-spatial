...
import model.entities.{Entity, EntityType, SpatialEntity}
...
case class SedonaPartitioner(source: SpatialRDD[Geometry], partitions: Int, gt: Constants.SedonaGridType.SedonaGridType = Constants.SedonaGridType.QUADTREE) {

  lazy val gridType: GridType = gt match {
    case Constants.SedonaGridType.KDBTREE => GridType.KDBTREE
    case Constants.SedonaGridType.QUADTREE => GridType.QUADTREE
  }
